...
#PYTHONPATH=.:$PYTHONPATH python3 $2 --datasets "${datasets[@]}" --experiment_type "default"
PYTHONPATH=.:$PYTHONPATH python3 $2 --datasets "${datasets[@]}" --experiment_type "mask" --knowledge "conceptnet" --head_mask_type "none" \
                                    --scoring_type "default" --loss_type "default" --learn_pos_embed_encoder "yes" --use_context "yes" --use_extra_relations "yes" \
                                    --use_dynamic_heads "yes" --pre_model "facebook/bart-large" --epochs 10 --batch_size 128 --max_length 64
