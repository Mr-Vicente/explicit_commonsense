...
srun -n1 -c4 --job-name="bart_default" --mem=30000 --time=0 --gres=gpu:3g.20gb:1 python $2 "${args[@]}" &
  ...
  do
    ...
    for mask_type in "${mask_types[@]}"
      do
          ...
          srun -n1 -c4 --job-name="bart_${kg_type}_${mask_type}" --mem=80000 --time=0 --gres=gpu:3g.20gb:1 python $2 "${args[@]}" &
          sleep 3
      done
